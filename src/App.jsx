...
import AddPost from "./components/ui/buttons/AddPost";
...
function App() {
  ...
  return (
    <div className={`w-full min-h-screen flex flex-col relative ${theme.bg}`}>
      ...
      {modalDelete && (
        <ModalConfirmation
          confirmar={async () => {
            const token = localStorage.getItem("token");
            await deletarPosts(currentPost.id, token);
            await carregarPostagens ();
            setModalDelete(false)
          }}
          cancelar={() => setModalDelete(false)}
        />
      )}
          ...
          <div className="w-full z-100 flex flex-col gap-5 relative max-w-4xl p-7 bg-white rounded-md shadow-md overflow-hidden">
            ...
            <div className="flex flex-col gap-3">
              ...
              <input
                ...
                value={tituloPost}
                className={`${tentouEnviar && tituloPost.trim() === "" ? "ring-2 ring-red-500" : "ring-0"} w-full border p-2 outline-0`}
              ...
              <p>Descricao:</p>
              <input
                onChange={(e) => setDescricaoPost(e.target.value)}
                value={descricaoPost}
                className={`${tentouEnviar && descricaoPost.trim() === "" ? "ring-2 ring-red-500" : "ring-0"} w-full border p-2 outline-0`}
            ...
            <div className="flex justify-center">
              ...
              >
                <p className="font-medium">
                  {modo === "adicionar" ? "Enviar post" : "Enviar alterações"}
                </p>
      ...
      <PostsSection
        ...
        setDescricaoPost={setDescricaoPost}
        setModal={setModal}
        setModo={setModo}
